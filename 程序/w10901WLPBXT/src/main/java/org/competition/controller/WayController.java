...
@Controller
@Scope("prototype")
@RequestMapping(value = {"/backend/way", "/way"})
public class WayController extends BaseController {
    ...
    @RequestMapping("/add")
    @ResponseBody
    public Map<String, Object> insertMatchWay(Way way) {
        Map<String, Object> map = new HashMap<>();
        try {
            wayService.insert(way);
            map.put("code", 1);
            map.put("状态", "成功");
        } catch (Exception e) {
            System.out.println("赛道插入失败" + e);
            map.put("code", 0);
            map.put("状态", "失败");
        }
        return map;
    ...
    public String list() {
        ...
        PageContext.getPage().setPageNo(pageNo);
        PageContext.getPage().setPageSize(pageSize);
        request.setAttribute("wayList", wayService.search());
        return "backend/way/way_list";
    }
