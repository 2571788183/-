...
<c:if test="${login_user.role.id eq '1'}">
		<div>
			...
			<input type="text" name="settime" id="settime" value="${production.settime }" size="8" maxlength="200" /> 
			<br />	
		</div>
</c:if>
<c:if test="${login_user.role.id ne '1'}">
		...
		<div>
			...
			<input type="hidden" name="docts" id="docts" value="${production.docts }" size="60" maxlength="200" /> 
		...
		<div>
			...
			<textarea rows="10" cols="100" name="content" id="content" >${production.content }</textarea>
			</div>
			<br />	
		</div>
		<div>
