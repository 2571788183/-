...
<c:if test="${login_user.role.id eq '1'}">
		<div>
			...
			<input type="text" name="settime" id="settime" value="${signup.settime }" size="8" maxlength="200" /> 
			<br />	
		</div>
</c:if>
<c:if test="${login_user.role.id ne '1'}">
		...
		<div>
			...
			<textarea rows="10" cols="100" name="content" id="content" >${signup.content }</textarea>
			</div>
			<br />	
		</div>
		<div>
...
<c:if test="${login_user.role.id ne '2'}">
		<div>
			...
			<select name="member" id="memberIds" >
				...
				<option value="${s.id }"  <c:if test="${s.id eq signup.member.id }">selected="selected"</c:if>
				>${s.name}</option>
				</c:forEach>
			</select> 
		</div>
