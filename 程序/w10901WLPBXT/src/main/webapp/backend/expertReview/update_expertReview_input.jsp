	...
	<fieldset>
		...
		<div>
			...
			<select name="production" id="productionIds" >
				...
				<option value="${s.id }"  <c:if test="${s.id eq expertReview.production.id }">selected="selected"</c:if>
				>${s.name}</option>
				</c:forEach>
			</select> 
		</div>
<c:if test="${login_user.role.id eq '1'}">
		<div>
			...
			<input type="text" name="settime" id="settime" value="${expertReview.settime }" size="8" maxlength="200" /> 
			<br />	
		</div>
</c:if>
...
<c:if test="${login_user.role.id eq '1'}">
		<div>
			...
			<textarea rows="10" cols="100" name="yijiancontent" id="yijiancontent" >${expertReview.yijiancontent }</textarea>
			</div>
			<br />	
		</div>
		<div>
